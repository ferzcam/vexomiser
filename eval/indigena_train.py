...
augmented with gene-HP and disease-HP associations from training cases.
...
Graph options (cumulative):
  ...
  Graph 2: + gene → has_phenotype → HP  (from training cases)
...
Usage (run from vexomiser root):
    ~/miniforge3/envs/indigena/bin/python eval/indigena_train.py \\
        --upheno-edges /path/to/upheno_owl2vecstar_edges.tsv \\
...
def build_graph(upheno_edges_path: str,
    ...
    gene2pheno:    gene_symbol → list of HP IRIs (from training cases)
    ...
    # ---- gene2pheno: aggregate over training cases ----
    ...
    for _, row in train_cases.iterrows():
        ...
        for hp in row["hpo_list"]:
            ...
            if iri not in graph1_entities:
                continue        # skip HP terms not in UPheno projection
    ...
    if graph2:
        logger.info("Adding Graph-2 edges (gene → HP)...")
        n = 0
        for gene, phenos in gene2pheno.items():
            ...
            for p in phenos:
                ...
                n += 1
        logger.info(f"  {n:,} gene-HP triples added")
...
@ck.option("--upheno-edges", required=True,
           help="Path to pre-computed UPheno OWL2VecStar edges TSV "
...
@ck.option("--graph2", is_flag=True, help="Add gene-HP edges (Graph 2).")
...
def main(upheno_edges, track, eval_split, graph2, graph3, graph4,
    ...
    if not only_eval:
        ...
        # Rebuild graph to reconstruct triples_factory (needed for entity vocab)
        triples, gene2pheno, disease2pheno = build_graph(
            upheno_edges, train_cases, test_disease_ids,
            graph2, graph3, graph4
        )
    ...
    print(f"\n=== INDIGENA — Track {track}  Graph {graph_tag}  {eval_split} ===")
