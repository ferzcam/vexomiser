...
MODES = ["baseline", "incomplete_3", "noisy_2", "imprecise_2", "superclass_all", "superclass_k1", "superclass_k2", "superclass_k3"]
...
def main():
    ...
    out_tsv = os.environ.get("PERTURB_ANALYSIS_OUT", os.path.join(RESULTS_DIR, "perturbation_analysis.tsv"))
    ...
    for track in [1, 2]:
        ...
        for metric in HEADLINE:
            piv = tdf.pivot_table(index="label", columns="mode", values=metric, observed=False).reindex(columns=MODES)
    ...
    report = os.environ.get("PERTURB_REPORT_OUT", os.path.join(RESULTS_DIR, "perturbation_report.md"))
